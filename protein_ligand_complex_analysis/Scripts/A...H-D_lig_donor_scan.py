...
filetxt=open(sys.argv[1]) 
txt_lines=filetxt.read().split('\n') 
filetxt.close()
#fileout=open('out_A...H-D_lig_donor_scan.txt','w')
fileout=open(sys.argv[2],'w')
#f1=open('error_A...H-D_lig_donor_scan.txt','w')
...
for ppp in range(lenlines):
        ...
        for ln in lines:
            if len(ln)>=6 and (ln[0:4]=='ATOM' or ln[0:6]=='HETATM'):
                if ln[0:4]=='ATOM' :
                    ...
                    if atm.rtype=='ASN' and (modelno=='1' or modelno=='A' or modelno==[]) :
                        if atm.atype=='ND2' or atm.atype=='OD1':
                            A.append(atm)
                    if atm.rtype=='GLN' and (modelno=='1' or modelno=='A' or modelno==[]) :
                        if atm.atype=='NE2' or atm.atype=='OE1':
                            A.append(atm)
                    if atm.rtype=='HIS' and (modelno=='1' or modelno=='A' or modelno==[]) :
                        if atm.atype=='NE2' or atm.atype=='ND1':
                            A.append(atm)
                    ...
                    if atm.rtype=='ARG' and (modelno=='1' or modelno=='A' or modelno==[]) :
                        if atm.atype=='NH1' or atm.atype=='NH2'  or atm.atype=='NE':
                            A.append(atm)
                    if atm.rtype=='LYS' and (modelno=='1' or modelno=='A' or modelno==[]) :
                        if atm.atype=='NZ':
                            A.append(atm)
                    ...
                    if atm.rtype=='TYR' and (modelno=='1' or modelno=='A' or modelno==[]) :
                        if atm.atype=='OH':                    
                            A.append(atm)
                    if atm.rtype=='ASP' and (modelno=='1' or modelno=='A' or modelno==[]) :
                        if atm.atype=='OD1' or atm.atype=='OD2':
                            A.append(atm)
                    if atm.rtype=='GLU' and (modelno=='1' or modelno=='A' or modelno==[]) :
                        if atm.atype=='OE1' and atm.atype=='OE2':
                            A.append(atm)
                ...
                if ln[0:6]=='HETATM' :
                    ...
                    atm.symb=ln[76:78].strip()
                    #print(atm.aid)     
                    if (atm.symb=='O' or atm.symb=='N' or atm.symb=='C' or atm.symb=='S' ) and (modelno=='1' or modelno=='A' or modelno==[]) :
                        D.append(atm) 
                    if (atm.symb=='H' ) and (modelno=='1' or modelno=='A' or modelno==[]) :
                        H.append(atm)      
    ...
    for d in range(len(D)):
        for h in range(len(H)):
            if D[d].rid==H[h].rid and D[d].chainid==H[h].chainid :
                for a in range(len(A)):
                    if getlen(H[h],A[a])<= getvanderwaalsum(H[h],A[a]) and getlen(H[h],D[d])< 1.25 and getangle(D[d],H[h],A[a])>=90 :
                    
                        intr.append([])
                        intr[len(intr)-1].append(filename)                                                     
                        intr[len(intr)-1].append(A[a].chainid)
                        intr[len(intr)-1].append(A[a].rtype)                
                        intr[len(intr)-1].append(A[a].rid)
                        intr[len(intr)-1].append(A[a].aid)
                        intr[len(intr)-1].append(A[a].atype)
                        intr[len(intr)-1].append(H[h].chainid)
                        intr[len(intr)-1].append(H[h].rtype)
                        intr[len(intr)-1].append(H[h].rid)
                        intr[len(intr)-1].append(H[h].atype)
                        intr[len(intr)-1].append(D[d].atype) 
                        intr[len(intr)-1].append(D[d].aid)    
                        intr[len(intr)-1].append(getlen(A[a],H[h]))           
                        intr[len(intr)-1].append(getangle(D[d],H[h],A[a]))
    ...
    intr=[]
    fileout.close()
    #fileout=open('out_A...H-D_lig_donor_scan.txt','a')
