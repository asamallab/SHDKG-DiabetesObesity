...
filetxt.close()
#fileout=open('out_binding_site_scan.txt','w')
fileout=open(sys.argv[2],'w')
#f1=open('error_binding_site_scan.txt','w')
...
for ppp in range(lenlines):
    ...
    for l in range(len(L)):
        for p in range(len(P)): 
            try:
                if getlen(L[l],P[p])<= getvanderwaalsum(P[p],L[l]) :
                    intr.append([])
                ...
                print ('Error:',l,p,P[p],L[l], filename)
    ...
    intr=[]
    fileout.close()
    #fileout=open('out_binding_site_scan.txt','a')
