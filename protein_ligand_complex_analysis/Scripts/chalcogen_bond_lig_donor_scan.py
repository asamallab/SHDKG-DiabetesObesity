...
filetxt.close()
fileout=open('out_chalcogen_bond_lig_donor_scan.txt','w')
f1=open('error_chalcogen_bond_lig_donor_scan.txt','w')
intr=[]
...
for ppp in range(lenlines):
            ...
            if len(ln)>=6 and (ln[0:4]=='ATOM' or ln[0:6]=='HETATM'):
                ...
                if ln[0:6]=='HETATM' :
                    ...
                    if (atm.symb=='C') and (modelno=='1' or modelno=='A' or modelno==[]) :
                        C.append(atm) 
                    if (atm.symb=='S' or atm.symb=='Se') and (modelno=='1' or modelno=='A' or modelno==[]) :
                        S.append(atm)      
    ...
    intr=[]
    fileout.close()
    fileout=open('out_chalcogen_bond_lig_donor_scan.txt','a')
fileout.close()
f1.close()
