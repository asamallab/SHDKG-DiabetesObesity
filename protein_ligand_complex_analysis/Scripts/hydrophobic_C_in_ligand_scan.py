...
filetxt.close()
fileout=open('out_hydrophobic_C_in_ligand_scan.txt','w')
f1=open('error_hydrophobic_C_in_ligand_scan.txt','w')
intr=[]
...
for ppp in range(lenlines):
        ...
        for ln in lines:
            if len(ln)>=6 and (ln[0:4]=='ATOM' or ln[0:6]=='HETATM'):
                if ln[0:4]=='ATOM' :
                    ...
                    if (atm.symb=='C'  or atm.symb=='S')and (modelno=='1' or modelno=='A' or modelno==[]) :
                        P.append(atm)  
    ...
    intr=[]
    fileout.close()
    fileout=open('out_hydrophobic_C_in_ligand_scan.txt','a')
fileout.close()
f1.close()
